...
app.delete("/todos/:id", (req, res) => {
  ...
  fs.readFile("todos.json", "utf8", (err, data) => {
    ...
    let updatedTodotodos = todos.filter((todo) => (todo.id !== parseInt(req.params.id)));

    if (todos.length === updatedTodotodos.length) res.status(404).send();

    fs.writeFile(
      "todos.json",
      JSON.stringify(updatedTodotodos),
      "utf8",
      (err) => {
        if (err) throw err;
